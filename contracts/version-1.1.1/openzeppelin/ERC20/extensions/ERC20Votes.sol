...
contract ERC20Votes is ERC20Permit {
     ...
     */
    function checkpoints(address account, uint32 pos) public view  returns (uint32, uint224) {
        Checkpoint memory checkpoint = _checkpoints[account][pos];
        uint32 fromBlock = checkpoint.fromBlock;
        uint224 votes = checkpoint.votes;
    ...
    function _writeCheckpoint(
        ...
        if (pos > 0 && ckpts[pos - 1].fromBlock == block.number) {
            ...
            ckpts.push(Checkpoint({fromBlock: SafeCast.toUint32(block.number), votes: toUint224(newWeight)}));
        }
    }
    ...
    function toUint224(uint256 value) internal pure returns (uint128) {
